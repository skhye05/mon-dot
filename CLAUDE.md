...
**`OTHER_EXPENSES`** (also in `lib/dot-data.ts`) is a separate, static ledger rendered by `other-expenses.tsx` ("Autres dépenses" — pré-dot cash + the Santa Lucia receipt). It is **deliberately NOT folded into the dot KPIs**: the receipt's jus + vin are the source for items #15/#17, so counting them in `compute()` would double-count. Keep it standalone.
